...
serve(async (req) => {
  ...
  try {
    ...
    // Use LLM to refine and complete the DSL
    const llmProvider = selectLLMProvider();
...
Output valid JSON with 'dsl', 'warnings', 'confidence', and optionally 'alternatives' fields.`;
    ...
    const llmResponse = await llmProvider.generateCompletion(llmPrompt, ENHANCED_SYSTEM_PROMPT);
